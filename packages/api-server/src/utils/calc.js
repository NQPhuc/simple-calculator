...
function plus (op1, op2) {
  ...
  let j = b.length - 1;
  for (; j > -1; j -= 1, i -= 1) {
    const digitAndCarry = getDigitAndCarry(parseInt(a[i], 10) + parseInt(b[j], 10) + carry, 10);
    // adding from right to left (new digit is append to the left of the current result string)
    res = digitAndCarry[0] + res;
    carry = digitAndCarry[1];
  }

  for (; i > -1; i -= 1) {
    const digitAndCarry = getDigitAndCarry(parseInt(a[i], 10) + carry, 10);
    // adding from right to left (new digit is append to the left of the current result string)
    res = digitAndCarry[0] + res;
    carry = digitAndCarry[1];
  }
...
function minus (op1, op2) {
  ...
  let j = b.length - 1;
  for (; j > -1; j -= 1, i -= 1) {
    const digitAndCarry = getDigitAndBorrow(parseInt(a[i], 10) - parseInt(b[j], 10) - borrow, 10);
    // adding from right to left (new digit is append to the left of the current result string)
    res = digitAndCarry[0] + res;
  ...
  for (; i > -1; i -= 1) {
    const digitAndCarry = getDigitAndBorrow(parseInt(a[i], 10) - borrow, 10);
    // adding from right to left (new digit is append to the left of the current result string)
    res = digitAndCarry[0] + res;
    borrow = digitAndCarry[1];
  }
...
function mod (a, b) {
  ...
  const remain = trimZero(minus(a, B));
 ...
 * @returns
 */
export async function calculate (op1, op2, op) {
  let res = -999;
  let [a, b] = [op1, op2];
